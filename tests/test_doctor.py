...
from zerion_orchestration.doctor import capability_report
...
class RuntimeDoctorTests(unittest.TestCase):
    def test_report_is_machine_readable_and_does_not_emit_secret_values(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "coordination" / "zerion.yaml").write_text("schema: 1\n")
            ...
            self.assertEqual(report["kind"], "zerion-runtime-capability-report")
            self.assertTrue(report["local_repository"]["zerion_registry_present"])
    ...
    def test_doctor_json_cli_succeeds_even_when_optional_tools_are_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = io.StringIO()
            with mock.patch("zerion_orchestration.doctor.shutil.which", return_value=None):
