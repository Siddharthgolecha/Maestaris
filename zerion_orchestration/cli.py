...
def command_init(args: argparse.Namespace) -> int:
    ...
    if registry.get("protocol_version") != 4:
        print("existing Zerion registry is not protocol v4; migrate before init", file=sys.stderr)
        return 2
...
def command_validate(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "quiet", False):
        print(f"Zerion configuration validation OK: {len(result.projects)} project(s), {len(result.agents)} agent(s)")
    return 0
...
def command_status(args: argparse.Namespace) -> int:
    ...
    result = validate_repository(root)
    if args.json:
        print(json.dumps({"ok": result.ok, "errors": result.errors, "warnings": result.warnings, "registry": result.registry, "projects": result.projects, "agents": result.agents, "live_state": "github"}, indent=2, sort_keys=True))
    ...
    for name, project in sorted(result.projects.items()):
        ...
        print(f"{name[:23]:23}  {str(project.get('status'))[:10]:10}  {workers:>7}  {project.get('repository', '-')}")
    if result.warnings:
        print("\nWarnings:")
        for warning in result.warnings:
            print(f"- {warning}")
...
def command_audit(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        findings = [(item["issue"], finding) for item in report["tasks"] for finding in item["findings"]]
        globals_ = report["global_findings"]
        print(f"Zerion audit: {len(findings) + len(globals_)} finding(s)")
        for issue, finding in findings:
            print(f"- issue #{issue}: {finding['severity']} {finding['code']}: {finding['message']}")
        for finding in globals_:
            print(f"- repository: {finding['severity']} {finding['code']}: {finding['message']}")
        if args.fix:
            print(f"Safe derived repair plan: {len(report.get('repairs', []))} label update(s); canonical history untouched")
    has_error = any(f["severity"] == "error" for item in report["tasks"] for f in item["findings"]) or any(f["severity"] == "error" for f in report["global_findings"])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init.add_argument("--workers", nargs="+", default=["theory", "implementation", "audit"], help="worker roles; IDs are namespaced as project-role")
    init.add_argument("--repository", help="GitHub owner/repository; inferred from local git remote when possible")
    ...
    audit = sub.add_parser("audit", help="audit a GitHub-derived project snapshot without rewriting canonical history")
    audit.add_argument("--input", required=True, help="JSON snapshot of tasks/topology/optional Project state")
    audit.add_argument("--json", action="store_true")
    audit.add_argument("--fix", action="store_true", help="emit a safe derived-state repair plan only")
    audit.add_argument("--dry-run", action="store_true", help="mark repair output explicitly as dry-run")
