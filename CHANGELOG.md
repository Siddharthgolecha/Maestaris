...
- Add a reusable GitHub workflow for consumer repositories to validate Maestaris protocol events and synchronize derived Issue labels.
- Allow Maestaris runtime scripts to operate against an external consumer repository root without copying implementation scripts into that repository.
...
- Introduce Maestaris protocol v4 queue semantics.
...
- Replace `maestaris:assigned` with `maestaris:ready`.
...
- Bump the Maestaris protocol to v3.
...
- Add GitHub Action synchronization of `maestaris:*` status labels and `priority:*` labels.
...
- Make GitHub Issues the preferred Maestaris task/control-plane primitive.
...
- Add structured Maestaris task Issue template.
- Add GitHub Actions validation for Maestaris task Issues and protocol comments.
...
- Make root `AGENTS.md` the normative Maestaris entry point for AI agents.
- Add `coordination/maestaris.yaml` as the global protocol, pool, defaults, and project registry.
